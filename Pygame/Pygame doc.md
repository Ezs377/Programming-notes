...
An image can be considered a Surface object, as a Surface is basically a graphical 'surface' of the screen. Like the different layers in digital art, each surface provides the user a graphical representation of the game. The main difference between an image and a sprite is that an iamge is just an uploaded set of pixel data, which is jsut another Surface, whereas a sprite is an object that is both an image with a Rect() object.

Images imported as they are can only be affected with methods and functions that work with images, such as blit(). However, in order to the images to actually be recognized as an object by Pygame, they have to be associated with a Rect() object. A Rect() object is just a rectangular area that Pygame recognizes as 'interactable', which lets you things such as collision detection and movement. Sprites are just type of object created with the Sprite class, which merges an image and a Rect() object together for you. It also lets you use Sprite methods and functions which may make it easier to program. Hoeever, Sprites aren't necessary in Pygame as images don't always need a Rect() object (e.g. Background images), and you can also just manually associate a custom Rect() objetc with an image. The Sprite class is just a wrapper.
